			...
			if (isAdminGhost(usr) || (I && istype(I)))
				if(isAdminGhost(usr) || (access_heads in I.access)) //Let heads change the alert level.
					...
					if(tmp_alertlevel > SEC_LEVEL_SUNSET)
						tmp_alertlevel = SEC_LEVEL_SUNSET //Cannot engage supernova protocol with this
		...
		if("SetPortRestriction")
			...
			if (I || isAdminGhost(usr))
				if(isAdminGhost(usr) || (access_hos in I.access) || (access_heads in I.access && security_level >= SEC_LEVEL_NIGHTFALL))
					...
					to_chat(usr, "<span class='warning'>This action requires either nightfall protocol or head of security authorization.</span>")
			else
...
/obj/machinery/computer/communications/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open=NANOUI_FOCUS)
	...
	data["levels"] = list(
		...
		//SEC_LEVEL_NIGHTFALL = list("name"="Nightfall Protocol"),
	)
